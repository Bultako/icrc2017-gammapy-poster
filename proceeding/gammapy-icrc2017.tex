...
Astropy \cite{lsst}, and that a huge eco-system of scientific Python packages
becomes available for advanced users if the data is exposed as Numpy arrays
(e.g. healpy, pandas, Scipy, scikit-image, scikit-learn, iminuit, emcee,
\ldots).
...
An example script using Gammapy is shown in Figure~\ref{fig:code_example}, that
generates a counts image from an event list. The remarkable point we would like
to make here is that it is possible to efficiently work with events and pixels
and to implement algorithms from Python, by storing all data in Numpy arrays and
processing via calls into existing C extensions in Numpy and Astropy. E.g. here
EventList stores the RA and DEC columns from the event list as arrays, and
SkyImage the pixel data as well, and {\it image.fill(events)}, and all
processing happens in existing C extensions (Numpy histograms and Astropy calls
into the CFITSIO and WCSLib C libraries). Should the need arise, that some
algorithm can’t be efficiently implemented in pure Python, a small C extension
can be added to Gammapy, using e.g. Cython \cite{cython}.

Gammapy is high-level Python, data in Numpy arrays, others have written
C and Python wrappers already. Specifically, {\it astropy.wcs} is calling into
WCSLib (\cite{wcslib}), {it astropy.io.fits} uses CFITSIO (\cite{cfitsio}) and
{\it astropy.coordinates} as well as {\it astropy.time} are built on ERFA
(\url{https://github.com/liberfa/erfa}), the open-source variant of this IAU
Standards of Fundamental Astronomy (SOFA) C library
...
For testing we use pytest (\url{https://pytest.org}). For documentation Sphinx
(\url{http://www.sphinx-doc.org}), for tutorial-style documentation Jupyter
notebooks (\url{https://jupyter.org/}).
...
June 2016 and Paris in Feb 2017). TODO: mention code review, continuous
integration, coverage and other code quality things.
